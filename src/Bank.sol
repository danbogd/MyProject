...
contract MyBank {
  ...
  receive() external payable {}
    ...
    //event WithdrawTokensFromContract (uint256 indexed nonce, address token, uint256 amount, uint256 time);
    ...
    function withdrawTokensFromContract (address _token, uint256 _amount) external  onlyOwner {
        ...
        //emit WithdrawTokensFromContract(nonce, _token, _amount, block.timestamp);
    }
    ...
    function withdrawMaticFromContract(address payable _recipient, uint256 _amount) external onlyOwner{
        ...
        emit WithdrawMaticFromContract(nonce, _amount, block.timestamp);// TODO
    }
